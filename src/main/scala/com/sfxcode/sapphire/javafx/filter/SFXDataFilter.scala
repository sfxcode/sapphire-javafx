...
class SFXDataFilter[S <: AnyRef](items: ObjectProperty[ObservableList[SFXBean[S]]], pane: ObjectProperty[Pane])
  ...
  def updateItems(newItems: ObservableList[SFXBean[S]], resetFilter: Boolean = false): Unit = {
  ...
  def itemsProperty: ObjectProperty[ObservableList[SFXBean[S]]] = items

  def itemValues: ObservableList[SFXBean[S]] = items.getValue
  ...
  def addSearchField(
      name: String,
      propertyKey: String,
      filterType: FilterValue = SFXFilterType.FilterContainsIgnoreCase,
    ...
    controlFilterMap.keySet.foreach {
      case textField: TextField =>
        ...
        if (filter.length > 0) {
          ...
          filtered = filtered.replaceByFilteredValues(controlFilterMap(textField).asInstanceOf[SFXBean[S] => Boolean])
        }
      case searchBox: ComboBox[String] =>
        ...
        if (model.selectedIndexProperty().get() > 0) {
          ...
          filtered = filtered.replaceByFilteredValues(controlFilterMap(searchBox).asInstanceOf[SFXBean[S] => Boolean])
          logger.debug(item)
        }
      case _ =>
    }
  ...
  private def filterFunction(function: FilterValue, property: String, valueKey: String): SFXBean[S] => Boolean =
    if (function == SFXFilterType.FilterContains)
      containsFunction(property, valueKey)
    else if (function == SFXFilterType.FilterContainsIgnoreCase)
      containsLowerCaseFunction(property, valueKey)
    else if (function == SFXFilterType.FilterEquals)
      equalsFunction(property, valueKey)
    else if (function == SFXFilterType.FilterEqualsIgnoreCase)
